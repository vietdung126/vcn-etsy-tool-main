...
const { allItems: stores } = storeToRefs(linkDesignStore)
    ...
    onClick: row => {
      linkDesignStore
        ...
        .then(
          result =>
            result
              ? toast.success(`Đã xóa link design "${row.sku}-${row.link}".`)
              : toast.warning(`Không thể xóa link design "${row.sku}-${row.link}".`),
          showError,
        )
    },
  },
...
onMounted(() => {
  linkDesignStore.load().catch(showError)
})
